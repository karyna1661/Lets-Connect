...
export function NavCard({ icon: Icon, title, description, backTitle, backDescription, backExtra, ctaLabel = "Let's Go", secondaryLabel, onSecondaryClick, onClick }: NavCardProps) {
  const frontContent = (
    <div
      className="w-full h-full bg-gradient-to-br from-white via-gray-50 to-white rounded-3xl border border-gray-200/50 overflow-hidden p-8 flex flex-col justify-center items-center text-center cursor-pointer group"
      style={{
        boxShadow: `
    ...
    >
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
        <div className="bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity">
          <span className="text-xs font-semibold text-gray-600">Click to continue →</span>
        </div>
      </div>
    </div>
  )
  ...
  const backContent = (
    <div
      className="w-full h-full rounded-3xl p-6 flex flex-col justify-between items-center text-center overflow-hidden cursor-pointer"
      style={{
    ...
    >
      ...
      <div className="flex-shrink-0 w-full max-w-[220px] space-y-2">
        <button
          onClick={(e) => {
            e.stopPropagation()
            onClick()
          }}
          className="px-6 py-2.5 bg-white text-black rounded-xl font-bold hover:bg-gray-100 transition-all shadow-lg w-full text-sm"
        >
          {ctaLabel}
        </button>
        ...
        {secondaryLabel && onSecondaryClick && (
          <button
            ...
            className="px-4 py-2 bg-white/10 text-white rounded-lg font-medium hover:bg-white/20 transition-all border border-white/20 w-full text-xs"
          >
            {secondaryLabel}
          </button>
        )}
